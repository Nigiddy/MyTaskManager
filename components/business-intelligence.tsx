...
export function BusinessIntelligence() {
  ...
  const [pipeline, setPipeline] = useState<ClientPipeline[]>([
    { stage: "Lead Generation", count: 24, value: 0, conversionRate: 0, color: "bg-blue-500" },
    { stage: "Initial Contact", count: 18, value: 0, conversionRate: 75, color: "bg-yellow-500" },
    { stage: "Demo/Pitch", count: 12, value: 0, conversionRate: 67, color: "bg-orange-500" },
    { stage: "Negotiation", count: 8, value: 0, conversionRate: 67, color: "bg-red-500" },
    { stage: "Closed Won", count: 6, value: 4200, conversionRate: 75, color: "bg-green-500" }
  ])

  const totalRevenue = metrics
    .filter(m => m.category === "Revenue")
    .reduce((sum, m) => sum + parseFloat(m.value.replace('$', '').replace(',', '')), 0)
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      ...
      case "Clients": return "bg-blue-100 text-blue-700 border-blue-200"
      case "Growth": return "bg-purple-100 text-purple-700 border-purple-200"
      case "Brand": return "bg-orange-100 text-orange-700 border-orange-200"
    ...
    <div className="bg-[#FFF8F3] rounded-xl p-4 shadow-[0_4px_12px_rgba(0,0,0,0.05)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="p-4 bg-white rounded-lg border border-[#FFE8D6] text-center">
          ...
          <div className="text-2xl font-bold text-[#333]">${totalRevenue.toLocaleString()}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        {metrics.map((metric) => (
          <div key={metric.id} className="p-3 bg-white rounded-lg border border-[#FFE8D6] hover:border-[#FF9F43] transition-colors">
            <div className="flex items-start justify-between mb-2">
              <div>
                <h3 className="text-sm font-medium text-[#333]">{metric.name}</h3>
                <Badge
                  variant="outline"
              ...
              <div className="text-right">
                <div className="text-lg font-bold text-[#333]">{metric.value}</div>
                <div className={`text-xs flex items-center ${getChangeColor(metric.changeType)}`}>
      ...
      <div className="mb-4">
        ...
        <div className="space-y-3">
          {pipeline.map((stage, index) => (
            <div key={stage.stage} className="flex items-center justify-between p-3 bg-white rounded-lg border border-[#FFE8D6]">
              <div className="flex items-center space-x-3">
                <div className={`w-3 h-3 rounded-full ${stage.color}`}></div>
                <div>
                  <span className="text-sm font-medium text-[#333]">{stage.stage}</span>
                  <div className="text-xs text-[#666]">
                    {stage.count} prospects • {stage.conversionRate > 0 ? `${stage.conversionRate}%` : 'N/A'} conversion
                  </div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-sm font-bold text-[#333]">{stage.count}</div>
                {stage.value > 0 && (
                  <div className="text-xs text-green-600">${stage.value.toLocaleString()}</div>
                )}
              </div>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-blue-700">Dem Man Brand</span>
            <Badge variant="outline" className="bg-blue-100 text-blue-700 border-blue-200 text-xs">
              Fitness
            </Badge>
          </div>
          <div className="text-lg font-bold text-blue-700">1,240 followers</div>
          <div className="text-xs text-blue-600 mt-1">+8.3% this month • Gym partnerships growing</div>
        </div>
        
        <div className="p-3 bg-gradient-to-r from-pink-50 to-rose-50 rounded-lg border border-pink-200">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-pink-700">Dicla Clothing</span>
          ...
          <div className="text-xs text-pink-600 mt-1">-5.2% this month • Need influencer strategy</div>
        </div>
      </div>
      ...
      <div className="mt-4 p-3 bg-gradient-to-r from-[#FFE8D6] to-[#FFF1E6] rounded-lg">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="font-bold text-[#FF9F43]">Scale • Optimize • Convert</span>
        </div>
        <div className="mt-2 text-xs text-[#666]">
          Focus on WiFi billing system completion • Increase Discord community engagement • Develop Dem Man gym partnerships
        </div>
      </div>
    </div>
  )
