...
import { Plus, Mic, Play, Pause, RotateCcw, Target, Zap, Clock, CheckCircle, X } from "lucide-react"
...
export function QuickActions() {
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      ...
      case "Timer": return "bg-green-100 text-green-700 border-green-200"
      case "Habit": return "bg-purple-100 text-purple-700 border-purple-200"
    ...
    <div className="bg-[#FFF8F3] rounded-xl p-4 shadow-[0_4px_12px_rgba(0,0,0,0.05)]">
          ...
          >
            <div className="text-center">
              ...
              <div className={`text-xs ${
                currentAction === action.name ? 'text-orange-100' : 'text-[#666]'
              }`}>
                {action.action}
              </div>
              {action.shortcut && (
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <Button
            ...
            className={`h-8 px-3 text-xs ${
              isRecording ? 'bg-red-600 hover:bg-red-700' : 'bg-[#FF9F43] hover:bg-[#FF8F33]'
            }`}
        ...
        {showVoiceInput && (
          <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200 mb-3">
            ...
            <div className="text-xs text-blue-600">
              {isRecording ? "Speak now... Recording your note" : "Processing your voice input..."}
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div className="p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
          ...
          <div className="text-xs text-green-600">
            Work without internet • Data syncs when online • Local storage enabled
          </div>
        </div>
        
        <div className="p-3 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg border border-purple-200">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-purple-700">Gesture Controls</span>
      ...
      <div className="mt-4 p-3 bg-gradient-to-r from-[#FFE8D6] to-[#FFF1E6] rounded-lg">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="font-bold text-[#FF9F43]">Speed • Efficiency • Growth</span>
        </div>
        <div className="mt-2 text-xs text-[#666]">
          Use voice notes during workouts • Keyboard shortcuts for power users • Offline mode for focus sessions
        </div>
      </div>
    </div>
  )
