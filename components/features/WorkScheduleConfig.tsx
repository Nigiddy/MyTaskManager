          ...
          <div className="space-y-2">
            {tempSchedule.workHours.map((hour, i) => (
              <div key={i} className="flex items-center space-x-2 p-2 bg-gray-50 rounded">
                <input type="time" value={hour.start} onChange={e => updateWorkHour(i, 'start', e.target.value)} disabled={!isEditing} className="border border-gray-300 rounded px-2 py-1 text-sm" />
                <span className="text-gray-500">to</span>
                <input type="time" value={hour.end} onChange={e => updateWorkHour(i, 'end', e.target.value)} disabled={!isEditing} className="border border-gray-300 rounded px-2 py-1 text-sm" />
                <input type="text" value={hour.label} onChange={e => updateWorkHour(i, 'label', e.target.value)} disabled={!isEditing} placeholder="Label" className="flex-1 border border-gray-300 rounded px-2 py-1 text-sm" />
                {isEditing && tempSchedule.workHours.length > 1 && (
                  <Button variant="ghost" size="sm" onClick={() => removeWorkHour(i)} className="text-red-600 hover:text-red-700 text-xs">×</Button>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            {tempSchedule.breaks.map((b, i) => (
              <div key={i} className="flex items-center space-x-2 p-2 bg-green-50 rounded">
                <input type="time" value={b.start} onChange={e => updateBreak(i, 'start', e.target.value)} disabled={!isEditing} className="border border-gray-300 rounded px-2 py-1 text-sm" />
                <span className="text-gray-500">to</span>
                <input type="time" value={b.end} onChange={e => updateBreak(i, 'end', e.target.value)} disabled={!isEditing} className="border border-gray-300 rounded px-2 py-1 text-sm" />
                <input type="text" value={b.label} onChange={e => updateBreak(i, 'label', e.target.value)} disabled={!isEditing} placeholder="Label" className="flex-1 border border-gray-300 rounded px-2 py-1 text-sm" />
                {isEditing && <Button variant="ghost" size="sm" onClick={() => removeBreak(i)} className="text-red-600 hover:text-red-700 text-xs">×</Button>}
              </div>
            ))}
          </div>
        </div>
