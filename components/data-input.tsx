...
export function DataInput() {
  const [selectedSection, setSelectedSection] = useState<string>("coding-sessions")
  ...
  const sectionEntries = entries.filter(entry => entry.sectionId === selectedSection)
  ...
  return (
    <div className="bg-gradient-to-br from-[#FFF8F3] to-[#FFE8D6] rounded-xl p-4 shadow-[0_4px_12px_rgba(0,0,0,0.05)] border border-[#FFE8D6]">
      <div className="flex items-center justify-between mb-4">
        ...
        <Badge variant="outline" className="bg-[#FFE8D6] text-[#666] border-[#FF9F43] text-xs">
                  ...
                  {Object.entries(entry.data).map(([key, value]) => {
                    ...
                    return (
                      <div key={key}>
                        ...
                        <span className="ml-1 font-medium text-[#333]">{String(value)}</span>
                      </div>
                    )
                  })}
                </div>
                <div className="text-xs text-[#666] mt-2">
                  {entry.updatedAt.toLocaleDateString()} at {entry.updatedAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </div>
              </div>
