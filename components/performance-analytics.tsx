...
import { BarChart3, TrendingUp, Clock, Target, Calendar, Activity, Zap, Brain } from "lucide-react"
...
export function PerformanceAnalytics() {
  ...
  const [focusSessions] = useState<FocusSession[]>([
    { id: 1, startTime: "6:00 AM", duration: 180, task: "Full-Stack Development", focusScore: 95, interruptions: 1 },
    { id: 2, startTime: "9:30 AM", duration: 90, task: "Python Learning", focusScore: 88, interruptions: 2 },
    { id: 3, startTime: "11:30 AM", duration: 150, task: "WiFi Billing System", focusScore: 92, interruptions: 0 },
    { id: 4, startTime: "2:30 PM", duration: 90, task: "Sales Outreach", focusScore: 78, interruptions: 3 },
    { id: 5, startTime: "6:30 PM", duration: 90, task: "UI/UX Design", focusScore: 85, interruptions: 1 }
  ])
  ...
  const averageFocusScore = Math.round(focusSessions.reduce((sum, session) => sum + session.focusScore, 0) / focusSessions.length)
  const totalInterruptions = focusSessions.reduce((sum, session) => sum + session.interruptions, 0)

  const currentWeek = weeklyMetrics[weeklyMetrics.length - 1]
  const previousWeek = weeklyMetrics[weeklyMetrics.length - 2]
  const productivityChange = currentWeek.productivity - previousWeek.productivity
    ...
    <div className="bg-[#FFF8F3] rounded-xl p-4 shadow-[0_4px_12px_rgba(0,0,0,0.05)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="p-3 bg-white rounded-lg border border-[#FFE8D6] text-center">
          ...
          <div className="text-xl font-bold text-[#333]">{currentWeek.productivity}%</div>
          <div className={`text-xs ${productivityChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            {productivityChange >= 0 ? '+' : ''}{productivityChange}% vs last week
          </div>
        </div>
        ...
        <div className="p-3 bg-white rounded-lg border border-[#FFE8D6] text-center">
          ...
          <div className="text-xl font-bold text-[#333]">{currentWeek.focusTime}h</div>
        ...
        <div className="p-3 bg-white rounded-lg border border-[#FFE8D6] text-center">
          ...
          <div className="text-xl font-bold text-[#333]">{currentWeek.tasksCompleted}</div>
        ...
        <div className="p-3 bg-white rounded-lg border border-[#FFE8D6] text-center">
          ...
          <div className="text-xl font-bold text-[#333]">{currentWeek.habitsMaintained}/7</div>
      ...
      <div className="mb-6">
        <h3 className="font-medium text-[#333] mb-3">Daily Productivity Heatmap</h3>
      ...
      <div className="mb-6">
        <h3 className="font-medium text-[#333] mb-3">Focus Sessions Analysis</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div className="p-3 bg-white rounded-lg border border-[#FFE8D6] text-center">
            <div className="text-2xl font-bold text-blue-600">{Math.round(totalFocusTime / 60)}h</div>
            <div className="text-xs text-[#666]">Total Focus Time</div>
          </div>
          <div className="p-3 bg-white rounded-lg border border-[#FFE8D6] text-center">
            <div className="text-2xl font-bold text-green-600">{averageFocusScore}%</div>
            <div className="text-xs text-[#666]">Average Focus Score</div>
          </div>
          <div className="p-3 bg-white rounded-lg border border-[#FFE8D6] text-center">
            <div className="text-2xl font-bold text-red-600">{totalInterruptions}</div>
        ...
        <div className="space-y-3">
          {focusSessions.map((session) => (
            <div key={session.id} className="p-3 bg-white rounded-lg border border-[#FFE8D6] hover:border-[#FF9F43] transition-colors">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-3">
                  <div className={`w-3 h-3 rounded-full ${getFocusScoreColor(session.focusScore)}`}></div>
                  <div>
                    <h4 className="text-sm font-medium text-[#333]">{session.task}</h4>
                    <div className="text-xs text-[#666]">{session.startTime} • {session.duration}m</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-bold text-[#333]">{session.focusScore}%</div>
                  <div className="text-xs text-[#666]">{getFocusScoreText(session.focusScore)}</div>
                </div>
              </div>
              
              <div className="flex items-center justify-between text-xs text-[#666]">
                <span>Interruptions: {session.interruptions}</span>
                <span>Focus Quality: {session.focusScore >= 90 ? '🔥 Excellent' : session.focusScore >= 80 ? '⚡ Good' : '💡 Fair'}</span>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            {weeklyMetrics.map((week, index) => (
              <div key={week.week} className="text-center">
                <div className="text-sm font-medium text-[#333] mb-2">{week.week}</div>
                <div className="space-y-2">
                  <div className="text-xs text-[#666]">Productivity</div>
                  <div className="text-lg font-bold text-[#FF9F43]">{week.productivity}%</div>
                  <Progress value={week.productivity} className="h-2 bg-[#FFE8D6]" />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="p-4 bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg border border-purple-200">
        ...
        <div className="mt-3 flex space-x-2">
          ...
          <Button variant="outline" size="sm" className="border-purple-300 text-purple-700 hover:bg-purple-50 text-xs">
      ...
      <div className="mt-4 p-3 bg-gradient-to-r from-[#FFE8D6] to-[#FFF1E6] rounded-lg">
        ...
        <div className="mt-2 text-xs text-[#666]">
          Peak productivity: 4-6 AM • Focus quality improves with fewer interruptions • Weekly reviews boost consistency by 15%
        </div>
      </div>
    </div>
  )
