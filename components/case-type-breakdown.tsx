    ...
    <div className="bg-[#FFF8F3] rounded-xl p-4 shadow-[0_4px_12px_rgba(0,0,0,0.05)]">
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="md:ml-4 mt-4 md:mt-0 space-y-3">
          {data.map((item) => (
            <div key={item.name} className="flex items-center justify-between p-2 bg-white rounded-lg border border-[#FFE8D6]">
              <div className="flex items-center">
                <div className="h-3 w-3 rounded-full mr-2" style={{ backgroundColor: item.color }}></div>
                <span className="text-sm font-medium">{item.name}</span>
              </div>
              <div className="text-right">
                <div className="text-sm font-bold text-[#333]">{item.value}%</div>
      ...
      <div className="mt-4 p-3 bg-gradient-to-r from-[#FFE8D6] to-[#FFF1E6] rounded-lg">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-bold text-[#FF9F43]">Stay Disciplined • Build Daily • Scale Up</span>
        </div>
      </div>
    </div>
  )
